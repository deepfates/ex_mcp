defmodule ExMCP.ACP.AdapterBridgeTest do
  ...
  # TreeAdapter: an agent that ignores SIGTERM and EOF and leaves a child
  # behind, recording both pids in the file named by :pid_file.
  defmodule TreeAdapter do
    ...
    def command(opts) do
      ...
      {System.find_executable("sh"), ["-c", script, Keyword.fetch!(opts, :pid_file)]}
    end
  ...
  @tag :unix
  @tag :unix
  test "closing the bridge ends its agent and the agent's children" do
    pid_file = Path.join(System.tmp_dir!(), "bridge-tree-#{System.unique_integer([:positive])}")
    on_exit(fn -> File.rm(pid_file) end)

    {:ok, bridge} =
      AdapterBridge.start_link(adapter: TreeAdapter, adapter_opts: [pid_file: pid_file])

    [leader_pid, child_pid] = read_pids(pid_file)
  ...
  test "closing an adapter subprocess confirms its process group is gone" do
    ...
    assert {:ok, process} =
             PortRunner.open_owned(shell, ["-c", script], [], MockAdapter)
  ...
  defp read_pids(path) do
    ExMCP.TestHelpers.wait_until(
      fn -> match?({:ok, <<_, _::binary>>}, File.read(path)) end,
      timeout: 5_000
    )
