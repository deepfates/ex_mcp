defmodule ExMCP.Internal.OwnedProcess do
  ...
  # erlexec is an optional dependency: a consumer that runs adapters declares it.
  ...
  def init({owner, executable, args, opts}) do
    ...
    # erlexec starts on the first spawn, not with ExMCP: its port program
    # refuses to start without SHELL, which a program spawned by an MCP host
    # often lacks, and most ExMCP users never spawn a process through this
    # module.
    with {:ok, _apps} <- start_erlexec(),
         {:ok, exec_pid, os_pid} <- :exec.run_link([executable | args], exec_opts) do
  ...
  defp start_erlexec do
    if Code.ensure_loaded?(:exec),
      do: Application.ensure_all_started(:erlexec),
      else: {:error, {:erlexec_not_installed, "add {:erlexec, \"~> 2.2\"} to run adapters"}}
  end
